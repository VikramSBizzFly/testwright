...
8. SEO cases, **on `--seo`**, generated by the engine at no cost:
   `tf.sh seo cases tests/.cache/routes.txt tests/.cache/privileged.txt > /tmp/seo.csv`,
   then `tf.sh merge /tmp/seo.csv`.
...
1. For every `page` case whose route has no `tests/.cache/pages/<route>.txt` —
   ...
   Leave out `tags=seo` cases: they never need a page model.
...
1. **`tf.sh run-api`** — `type=api` cases, over curl. Free. Always run these
   ...
   On `--seo`, **`tf.sh seo run`** next — also curl, also free. It judges
   every `tags=seo` case and leaves a client-rendered page UNJUDGED, listing
   it in `tests/.cache/seo/render.txt`.
...
3. **Browser** — everything else. Load the **execution** skill and hand
   ...
   `responsive-auditor` call per route, which checks 390/768/1280; on `--seo`,
   one `seo-auditor render <id> <route>` call per line of
   `tests/.cache/seo/render.txt`, then one `seo-auditor review` call with no
   browser. Fold their rows back with `tf.sh setmany` as for `test-runner`: a
   review returns only the cases it fails.
